...
import com.example.carecareforeldres.Service.ServiceMapBox;
...
public class AuthenticationService {
  ...
  private final ServiceMapBox mapbox;
    @Autowired
  ...
  public AuthenticationResponse register(User user) throws IOException {
    ...
    user.setNbr_tentatives(0);
      user.setAdresse( mapbox.getAddressFromCoordinates(user.getY(),user.getX()));
    ...
    private void saveUserToPatientTable(User savedUser) {
        {
            for (Role r:savedUser.getRoles()){
                ...
                if (r.getName()==TypeRole.MEDECIN){
                    String sql = "INSERT INTO medecin (user,disponible,specialite, nom, prenom, mail, x, y,adresse) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    jdbcTemplate.update(sql, savedUser.getId(), savedUser.getDisponiblee(), savedUser.getSpecialitee().toString(), savedUser.getFirstname(),savedUser.getLastname(), savedUser.getEmail(),savedUser.getX(),savedUser.getY(), savedUser.getAdresse());
                }
  ...
  private void revokeAllUserTokens(User user) {
    ...
    public Optional<?> getCurrentUsersWithRole(Integer id, String role) {
        switch (role) {
            ...
            case "ORGANISATEUR":
                return organisateurRepository.findOrganisateurByUser(id);
            default:
                return null;
        }
