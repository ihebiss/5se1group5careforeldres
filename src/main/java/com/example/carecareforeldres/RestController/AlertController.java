...
@GetMapping("/getalert/{idMedecin}")
    ...
    @GetMapping("/count-per-medecin")
    public ResponseEntity<List<MedecinAlertCountDTO>> countAlertsPerMedecin() {
        List<MedecinAlertCountDTO> resultList = iserviceAlert.countAlertsPerMedecin();
        return ResponseEntity.ok(resultList);
    }
    @PostMapping("/medecinevoueymail/{id}")
    public void envoyermail(@PathVariable("id")Integer id){
        iserviceAlert.envoyermail(id);
    }
}
