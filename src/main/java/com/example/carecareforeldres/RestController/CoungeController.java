    ...
    public ResponseEntity<?> DemandeCoungeCuisine(@PathVariable("idCuisinier") Integer CuisinierId, @RequestBody Counge counge) {
        ...
        try {
            ...
            if (counge.getDateFin().isBefore(counge.getDateDebut())) {
                if (differenceEnJours < var.getDifferenceEnJours()) {
                ...
                if (aPrisConge) {
                    if (counge.getTypeCounger().equals(TypeCounger.COUNGER_MATERNITE) && cuisinier.getSexe().equals(Sexe.FEMME) && dureeNouvelleConge <  var.getDureeNouvelleCongeMateral() && coungeRepository.countCongesByCuisinierAndYear(cuisinier, currentDate, TypeCounger.COUNGER_MATERNITE) == var.getZero()) {
                        coungeRepository.save(counge);
                        return getResponseEntityForMessage("Congé maternité ajouté avec succès", HttpStatus.OK);
                    } else if (counge.getTypeCounger().equals(TypeCounger.COUNGER_MATERNITE) && cuisinier.getSexe().equals(Sexe.FEMME) && dureeNouvelleConge >=  var.getDureeNouvelleCongeMateral()) {
                    ...
                    } else if (counge.getTypeCounger().equals(TypeCounger.COUNGER_MATERNITE) && coungeRepository.countCongesByCuisinierAndYear(cuisinier, currentDate, TypeCounger.COUNGER_MATERNITE) > var.getZero()) {
                        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Tu as déjà pris un congé maternité récemment");
                    }

                    if (counge.getTypeCounger().equals(TypeCounger.COUGER_PARENTALE) && cuisinier.getSexe().equals(Sexe.HOMME) && dureeNouvelleConge < var.getDureeNouvelleCongePaternal() && coungeRepository.countCongesByCuisinierAndYear(cuisinier, currentDate, TypeCounger.COUGER_PARENTALE) == var.getZero()) {
                        coungeRepository.save(counge);
                    ...
                    } else if (counge.getTypeCounger().equals(TypeCounger.COUGER_PARENTALE) && coungeRepository.countCongesByCuisinierAndYear(cuisinier, currentDate, TypeCounger.COUGER_PARENTALE) > var.getZero()) {
                ...
                if (joursCongesCetteAnnee + dureeNouvelleConge > var.getDureeNouvelleCongeParAns()) {
                    if (counge.getTypeCounger().equals(TypeCounger.COUNGER_MATERNITE) && coungeRepository.countCongesByCuisinierAndYear(cuisinier, currentDate, TypeCounger.COUNGER_MATERNITE) == var.getZero()) {
                        if (cuisinier.getSexe().equals(Sexe.FEMME) && dureeNouvelleConge < var.getDureeNouvelleCongeMateral()) {
                            coungeRepository.save(counge);
                    ...
                    if (counge.getTypeCounger().equals(TypeCounger.COUGER_PARENTALE) && cuisinier.getSexe().equals(Sexe.HOMME) && dureeNouvelleConge < 3 && coungeRepository.countCongesByCuisinierAndYear(cuisinier, currentDate, TypeCounger.COUGER_PARENTALE) == var.getZero()) {
                        coungeRepository.save(counge);
