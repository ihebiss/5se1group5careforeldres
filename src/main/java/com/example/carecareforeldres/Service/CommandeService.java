...
public class CommandeService implements ICommandeService {
    ...
    public User getUserAvecPlusDeCommandes() {
        ...
        Map<User, Integer> utilisateursCommandes = new HashMap<>();
        for (Commande commande : commandes) {
            User utilisateur = commande.getUser();
            utilisateursCommandes.put(utilisateur, utilisateursCommandes.getOrDefault(utilisateur, 0) + 1);
        }

        User utilisateurAvecPlusDeCommandes = Collections.max(utilisateursCommandes.entrySet(), Map.Entry.comparingByValue()).getKey();

        if (utilisateurAvecPlusDeCommandes != null) {
            System.out.println("L'utilisateur avec le plus grand nombre de commandes est : " + utilisateurAvecPlusDeCommandes.getFirstname() + " " + utilisateurAvecPlusDeCommandes.getLastname());
            System.out.println("Nombre de commandes : " + utilisateursCommandes.get(utilisateurAvecPlusDeCommandes));
        } else {
            System.out.println("Aucun utilisateur trouvé");
        }

        return utilisateurAvecPlusDeCommandes;
    }
    ...
    @Override
    public int getNombreDeCommandesDeUtilisateur(Integer Id) {
        return commandeRepository.countByUserId(Id);
    }
