...
repasRepository.save(repas);
    ...
    public ResponseEntity<?> AffecterRepasAUser(RepasAvecPlatsDTO repasDTO, Integer idPatient) {
        ...
        Integer nbIng=0;
        for(Integer idplat: repasDTO.getPlatsIds()){
            Plat p=platRepository.findById(idplat).get();
            for (Ingredient ingredient:p.getIngredients()){
                nbIng = ingredient.getQuantite();
                nbIng--;
                ingredient.setQuantite(nbIng);
    ...
    private float calculateEstimatedDailyCalories(Integer idPatient) {
        ...
        float nbCalorieEstimee = patient.getSexe().equals(Sexe.HOMME) ? (float) ((var.getPoid() * poid) + (var.getLongeur() * longeur) - (var.getAge() * age) + var.getCalHomme()) : (float) ((var.getPoid() * poid) + (var.getLongeur() * longeur) - (var.getAge() * age) - var.getCalFemme());
