...
//////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    @Override
    public void registerUserToEvent(Long eventId, Integer userId) {
    ...
    @Override
    public Map<TypeEnum, Float> calculateAverageParticipantsPerType() {
        Map<TypeEnum, Integer> totalParticipantsPerType = new HashMap<>();
        Map<TypeEnum, Integer> eventCountPerType = new HashMap<>();

        List<Evennement> evennements = evennementRepository.findAll();

        for (Evennement evennement : evennements) {
            TypeEnum type = evennement.getType();
            int remainingPlaces = evennement.getPlaceMax() - evennement.getUsers().size();

            totalParticipantsPerType.put(type, totalParticipantsPerType.getOrDefault(type, 0) + (evennement.getPlaceMax() - remainingPlaces));
            eventCountPerType.put(type, eventCountPerType.getOrDefault(type, 0) + 1);
        }

        Map<TypeEnum, Float> averageParticipantsPerType = new HashMap<>();
        for (TypeEnum type : totalParticipantsPerType.keySet()) {
            int totalParticipants = totalParticipantsPerType.get(type);
            int eventCount = eventCountPerType.get(type);
            float average = (float) totalParticipants / eventCount;
            averageParticipantsPerType.put(type, average);
        }

        return averageParticipantsPerType;
    }


    @Override
    public List<Object[]> getUsersWithEventCount() {
        return evennementRepository.findUsersWithEventCount();
    }


    @Override
    public List<String> getUsersNamesByEvent(Long eventId) {
        Evennement event = getEvennementById(eventId);
