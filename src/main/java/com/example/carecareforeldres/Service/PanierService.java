...
public class PanierService implements IPanierService {
 ...
 public Commande ajouterCommande(Integer id,Long idPanier, float montantTotal, ModePay modePay) {
  ...
  if (utilisateurAvecPlusDeCommandes.getId().equals(utilisateur.getId())) {
   // Appliquer la réduction de 15%
   montantTotal *= 0.85; // Réduction de 15%
  }
  ...
  List<Long> idProduits = panier.getProduits().stream()
          .map(Produit::getIdProduit)
          .collect(Collectors.toList());
  commande.setIdProduits(idProduits);
