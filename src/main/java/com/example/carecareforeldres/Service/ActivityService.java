   ...
   // }
    ...
    public void accepterActivite(Long idActivite) {
        ...
        if (activity != null) {
            ...
            // Vérifier si l'organisateur de l'activité est défini
            Organisateur organisateur = activity.getOrganisateur();
            if (organisateur != null) {
                // Si l'organisateur est défini, récupérer son identifiant
                int idH = organisateur.getUser();
                ...
                if (user != null) {
                    ...
                    log.error("L'organisateur de l'activité avec ID " + idActivite + " n'est pas trouvé dans la base de données.");
                }
            } else {
                log.error("L'organisateur de l'activité avec ID " + idActivite + " n'est pas défini.");
            }
        }
    }
...
@Override
    public List<Activity> getActivitiesForOrganisateur(int organisateurId) {
        // Récupérer l'organisateur par son identifiant
        Organisateur organisateur = organisateurRepository.findById(organisateurId)
                .orElseThrow(() -> new EntityNotFoundException("Organisateur non trouvé avec l'ID : " + organisateurId));

        // Récupérer les activités associées à cet organisateur
        return activityRepository.findByOrganisateur(organisateur);
    }
    @Override
    public List<Activity> getActivitiesByPatient(Integer patientId) {
        return activityRepository.getActivityByPatient(patientId);
    }
}
